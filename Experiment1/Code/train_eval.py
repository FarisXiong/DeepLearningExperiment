...
def train(config,model,train_iter,test_iter):
    ...
    modelCp.load_state_dict(torch.load(config.save_model_path))
    evaluateWithConfusion(config, modelCp, test_iter)
...
def evaluateWithConfusion(config,model,test_iter):
    model.eval()
    loss_total = 0
    predict_all = np.array([], dtype=int)
    labels_all = np.array([], dtype=int)
